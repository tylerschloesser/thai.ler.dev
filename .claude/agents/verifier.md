...
- Run the check exactly as stated: `pnpm lint`, `pnpm typecheck`, `pnpm build`, a targeted
  grep, or the `verify-offline` skill driven through `playwright-cli`.
...
- Never edit, never commit, and never run a command that changes state beyond what the check
  itself needs. Starting a dev or preview server for a browser check is fine; remember it
  reads and writes production data, and say so if you created anything.
