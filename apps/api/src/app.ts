...
export function createApp({ dispatcher }: { dispatcher: Dispatcher }): Hono {
  const app = new Hono()
  ...
  app.get('/api/state', async (c) => {
    ...
    return c.json(await getStore().readSince(getUserId(c), since))
  })
  ...
  app.post('/api/mutations', async (c) => {
    ...
    let result
    try {
      result = await getStore().applyMutations(userId, parsed.data)
    } catch (error) {
